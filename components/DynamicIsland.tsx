...
const DynamicIsland: React.FC = () => {
  ...
  const baseClasses = 'pointer-events-auto bg-black/95 backdrop-blur-2xl border border-white/10 shadow-2xl overflow-hidden flex flex-col items-center justify-center transition-all duration-500';
  const expandedClasses = 'w-[calc(100vw-3rem)] max-w-sm h-24 rounded-[2rem] p-6';
  const collapsedClasses = 'w-36 h-9 rounded-full cursor-pointer hover:w-44';
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                <div className="w-10 h-10 rounded-xl bg-zinc-900 border border-white/5 flex items-center justify-center overflow-hidden">
                  <div className="w-full h-full bg-gradient-to-br from-blue-600/20 to-transparent" />
                  <div className="absolute inset-0 flex items-center justify-center text-[8px] font-black text-white italic">BURV</div>
                </div>
                <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 border-2 border-black rounded-full shadow-[0_0_10px_rgba(34,197,94,0.8)]" />
              </div>
